...
Widget::Widget(QWidget *parent) : QWidget(parent), ui(new Ui::Widget) {
    ...
    if (QLibrary::isLibrary("F:/Munca/programare/atpFtdiRc522/libMPSSE.dll")) {
      QLibrary lib("F:/Munca/programare/atpFtdiRc522/libMPSSE.dll");
      if(lib.load()){
...
/*    if(NULL == h_libMPSSE) {
        qDebug("Failed loading libMPSSE.dll. Please check if the file exists in the working directory\n");
        exit(1);
    }
 ...
 * \warning
 */
FT_STATUS Widget::read_byte(uint8 slaveAddress, uint8 address, uint16 *data) {
 ...
 * \warning
 */
FT_STATUS Widget::write_byte(uint8 slaveAddress, uint8 address, uint16 data) {
    uint32 sizeToTransfer = 0;
